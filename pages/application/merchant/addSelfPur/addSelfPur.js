// pages/application/merchant/addSelfPur/addSelfPur.js
const app = getApp();
...
Page({
  ...
  getPurchaseDetail: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        wx.hideLoading();
        if (res.data.code == '1') {
          vm.setData({ orderDetail: vm.formatDetailData(res.data.data) })
        }
        else {
  ...
  getPurchaseList: function () {
    ...
    var url = app.globalData.url;
    wx.request({
      url: url + 'oms/ownPurReq/getOwnPurReqDetails',
      method: 'get',
      dataType: 'json',
      data: {
        openId: vm.data.openId,
  ...
  get7DayPurchase: function () {
    ...
    var url = app.globalData.url;
    wx.request({
      url: url + 'oms/selfReq/addPurchasereq',
      method: 'get',
      dataType: 'json',
      data: {
        openId: vm.data.openId,
  ...
  goShopList: function () {
    wx.navigateTo({
      url: '../selectShopList/selectShopList?source=1',//0：集采 1：自采 2：要货
    })
  },
  ...
  sumbitOrder: function () {
    ...
    var url = app.globalData.url;
    wx.request({
      url: url + 'oms/ownPurReq/saveOwnPurReq',
      data: {
        id: vm.data.id,
        openId: vm.data.openId,
