// pages/application/merchant/transfers/transfers.js
const app = getApp();
...
Page({
  ...
  getOrderList: function (refreshType) {
    ...
    wx.request({
      url: url + 'oms/purchasereq/getPurchasereqList',
      method: 'get',
      dataType: 'json',
      data: {
  ...
  getWorkFlow: function () {
    ...
    wx.request({
      ...
      data: {
        openId: vm.data.openId,
        businessType: 'BU0003'//自采 BU0001 集采 BU0002 要货 BU0003
      },
      success: function (res) {
        if (res.data.code == '1')
  ...
  goAddPur: function (e) {
    ...
    wx.navigateTo({
      url: '../addReqPur/addReqPur?templateId=' + flowId,
    })
  },
  ...
  addPurReq: function () {
    ...
    if (vm.data.workFlowList == null) {
      ...
      if (vm.data.workFlowList.length == 0) {
        wx.navigateTo({
          url: '../addReqPur/addReqPur',
        })
      } else if (vm.data.workFlowList.length == 1) {
        wx.navigateTo({
          url: '../addReqPur/addReqPur?templateId=' + vm.data.workFlowList[0].templateId,
        })
      } else {
  ...
  goDetail:function(e){
    ...
    var url = '../transfersDetail/transfersDetail?id=';
    if (billStatus == 2) {
      url = '../repostReqPur/repostReqPur?id=';
    }
    wx.navigateTo({
      url: url + id,
      success: function(res) {},
