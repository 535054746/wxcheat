...
Page({
  ...
  orderTodetails(e) {
    var item = {
      ...
      'receivetime': e.currentTarget.dataset.receivetime,
      'storename': e.currentTarget.dataset.storename,
      'moneyAmount': e.currentTarget.dataset.moneyamount
    }

    wx.navigateTo({
      url: '../../supplier/orderSaleDetail/orderSaleDetail?item=' + JSON.stringify(item),
    })
  },
  ...
  searchBtn() {
    ...
    if (than.data.inputVal == '') {
      ...
      wx.request({
        url: app.globalData.url + 'oms/goodsInspection/groupingItemGoodsInStoreId',
        method: 'POST',
        header: {
        ...
        data: {
          acceptance: 1,
          supplierId: than.data.supplierId,
          ...
          if (res.data.code == 1) {
            if (res.data.data == 0) {
              ...
              res.data.data.forEach(item => {
                item.deliveryTime = myUtil.formatTimeDate(new Date(item.deliveryTime))
              })
              var c = than.dataDeal(res.data.data)
  ...
  storePost(id, storeNum) {
    ...
    wx.request({
      ...
      data: {
        ...
        acceptance: 1,
        supplierId: than.data.supplierId,
        startTime: than.data.startTime,
        endTime: than.data.endTime,
      },
      success(res) {
        if (res.data.code == 1) {
          var contentList = than.data.conter
  ...
  dataDeal:function(data){
    ...
    data.forEach(function (el, index) {
      for (var i = 0; i < listArr.length; i++) {
        ...
        if (listArr[i].deliveryTime == el.deliveryTime) {
